...
namespace Academy
{
	public partial class Main : Form
	{
		...
		Query[] queries = new Query[]
		{
			new Query
				(
					"FORMATMESSAGE(N'%s %s %s', last_name, first_name, middle_name) AS N'ФИО Студента', birth_date, group_name, direction_name","Students, Groups, Directions", "[group]=group_id AND direction=direction_id"
				),
			new Query
				(
		...
		{
			$"Количество студентов: " /*+ (dgvStudents.RowCount - 1)*/,
			$"Количество групп: ",
		...
		private void tabControl_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			switch (tabControl.SelectedIndex)
			{
				...
				case 2:
					{
						//dgvDirections.DataSource = connector.Select("direction_name, COUNT(DISTINCT group_id) AS N'Количество групп', COUNT(DISTINCT stud_id) AS N'Количество студентов'", "Students, Groups, Directions", "[group]=group_id AND direction=direction_id", "direction_name");
						dgvDirections.DataSource = connector.Select("direction_name, COUNT(DISTINCT group_id) AS N'Количество групп', COUNT(DISTINCT stud_id) AS N'Количество студентов'", "Students  RIGHT JOIN Groups ON ([group]=group_id) RIGHT JOIN Directions ON (direction=direction_id)", "", "direction_name");
